        ...
        return <div className='font-muli'>
            ...
            <ul className='products flex justify-center list-none flex-wrap px-4'>
                {this.props.products.map(product => (
                    <li key={product._id} className='flex-0 w-screen sm:w-1/2 p-2 mr-auto'>
                         <div
          ...
          >
            <div className="lg:flex">
              ...
              <div className="lg:flex-shrink-0">
                <img onClick={()=>this.openModal(product)} className="rounded sm-rounded-lg lg:w-64 lg:h-64" src={product.image} alt='' />
              </div>
              {/* </a> */}
              <div className="flex flex-col items-center lg:items-start mt-4 lg:mt-0 lg:ml-4">
                <div
                  className="uppercase tracking-wide text-xs sm:text-md text-gray-800 font-bold lg:text-lg"
                >
            {product.title}
                </div>
                <div
                  className="block mt-1 lg:my-4 text-xs sm:text-md leading-tight text-gray-900"
                  >{formatCurrency(product.price)}</div>
                  <small className='hidden lg:block lg:text-left'>
                        {product.description}
                  </small>

                <button onClick={()=>this.props.addToCart(product)} className='primary w-auto sm:w-40 border-none bg-transparent sm:bg-green-400 text-gray-500 hover:text-gray-700 sm:text-white sm:hover:text-white sm:hover:bg-green-600 p-1 mt-2 text-xs sm:text-sm lg:mt-auto object-center focus:outline-none rounded-md'>
